...
export async function createA2AServer(config: Required<AgentConfig>): Promise<ServerHandle> {
  ...
  // Dynamic agent card handler — rewrites `url` to match the caller's Host
  // header so that clients behind Docker / reverse proxies reach the correct
  ...
  const serveAgentCard: RequestHandler = (req, res) => {
    ...
    res.json({ ...agentCard, url: `${proto}://${host}/a2a/jsonrpc` });
  };
  ...
  const httpServer = app.listen(port, hostname, () => {
    log.info("A2A server started", { bind: hostname, advertise: advertiseHost, port });
    console.log(`
╔══════════════════════════════════════════════════════════════╗
║                   OpenCode A2A Server                        ║
╠══════════════════════════════════════════════════════════════╣
...
║  Agent Card:    http://${advertiseHost}:${port}/${AGENT_CARD_PATH}
║  JSON-RPC:      http://${advertiseHost}:${port}/a2a/jsonrpc
║  REST API:      http://${advertiseHost}:${port}/a2a/rest
║  Context:       http://${advertiseHost}:${port}/context
║  Build Context: http://${advertiseHost}:${port}/context/build  [POST]
║  MCP Status:    http://${advertiseHost}:${port}/mcp/status
║  Health Check:  http://${advertiseHost}:${port}/health
╠══════════════════════════════════════════════════════════════╣
